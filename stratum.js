...
var StratumClient = function(options){
    ...
    function handleMessage(message){
        // console.log('message::'+JSON.stringify(message)) // debug
        switch(message.method){
            case 'mining.subscribe':
                handleSubscribe(message);
                break;
            case 'mining.extranonce.subscribe':
                // handleExtranonceSubscribe(message, true /*reply to socket*/); // hack
                break;
    ...
    function handleSubscribe(message){
        ...
        _this.emit('subscription',
            ...
            function(error, extraNonce1, extraNonce2Size){
                ...
                // console.log('extraNonce2Size::'+extraNonce2Size); // debug
                sendJson({
                    id: message.id,
                    result: [
                        [
    ...
    // no params
    function handleExtranonceSubscribe(message, replyToSocket){        
        if (replyToSocket) {
            sendJson({
                    id     : message.id,
                    result : true,
    ...
    function sendJson(){
        ...
        for (var i = 0; i < arguments.length; i++){
            // console.log(arguments[i]); // debug
            arguments[i]['jsonrpc'] = "2.0";
    ...
    this.sendMiningJob = function(jobParams){
        // console.log('clijobparams::'+JSON.stringify(jobParams)); // debug
        ...
        // set minining difficulty - check this with miner (should show on connect)                                                                                       
        // ?? using jobParams is a hack; should fix                                                                                                                       
        // if (typeof jobParams[6] !== 'string') {                                                                                                                        
            // removed multiplier; don't have abilility to make sure shares work out yet                                                                                  
            // if (options.multiplyDifficulty) {                                                                                                                          
            //     // var b = jobParams[6].multipliedBy(this.difficulty);                                                                                                 
            //     var b = jobParams[6].multipliedBy(2);                                                                                                                  
            //     jobParams[6] = b.toFixed(0);                                                                                                                           
            // } else { jobParams[6] = jobParams[6].toFixed(0);  }                                                                                                        
            jobParams[6] = BigInt(Math.pow(2, 256)/_this.difficulty).toString()                                                                                           
            // console.log('b::'+JSON.stringify(jobParams)) // debug                                                                                                      
        // }
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.handleNewClient = function (socket){
        ...
        // console.log(JSON.stringify(client)) // debug
        // console.log(JSON.stringify(stratumClients)); // debug
