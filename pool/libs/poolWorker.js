...
module.exports = function(logger){
    ...
    Object.keys(poolConfigs).forEach(function(coin) {
            ...
            handlers.auth = function(port, workerName, password, authCallback){
                if (poolOptions.validateWorkerUsername !== true)
                    ...
                    if (workerName.length === 40) {
                        ...
                        var worker_rig = workerName.split('.');
                        pool.daemon.cmd('/utils/address/'+ [worker_rig[0]], 'GET', [], function (results) {
        ...
        // console.log('a::'+JSON.stringify(logger)) // debug
        pool.on('share', function(isValidShare, isValidBlock, data){
            try {
                ...
                //console.log('YY::'+JSON.stringify(shareData))
                /*
                try {
                    var isoDate = new Date()
                    var dbStratum = new Client('postgresql://winter:t00lip@ergodb:5432/winter')
                    dbStratum.connect();
                    // testing .. dbStratum.query('select * from shares', (err, res) => {console.log(err, res); dbStratum.end()})
                    // `+Date().toLocaleString('sv', { timeZoneName: 'short' })+`
                    var [rigName, workerName] = shareData.worker.split('.')            
                    let sql = `insert into shares (rig, worker, is_valid, is_block_solution, miner_difficulty, network_difficulty, stratum
                                                    , status, block, job, ip, port, msg, nonce, extra, found_timestamp)
                        values (
                            '`+(rigName == undefined ? '' : rigName)+`', -- rig
                            '`+workerName+`', -- worker
                            1, -- isvalid
                            `+(isValidBlock ? 1 : 0)+`, -- isblocksoln
                            '`+shareData.difficulty+`', -- miner diff
                            '`+poolOptions.initStats.difficulty+`', -- network diff
                            'us-west1.tulipfarm.one', -- stratum
                            'unconfirmed', -- status
                            `+(isNaN(shareData.height) ? -1 : shareData.height)+`, -- block
                            '`+shareData.job+`', -- job
                            '`+shareData.ip+`', -- ip
                            `+(isNaN(shareData.port) ? -1 : shareData.port)+`, -- port
                            '`+shareData.msg+`', -- msg
                            '`+nonce+`', -- nonce
                            '`+JSON.stringify(shareData)+`', -- extra
                            '`+isoDate.toISOString()+`' -- found
                        )`;
                    dbStratum.query(sql, (err, res) => {
                        // console.log('SHR2DB::'+JSON.stringify(sql)+'::'+err, res)
                        dbStratum.end()                
                    })
                }
                catch (err) {
                    console.log('PGSHARE::'+JSON.stringify(sql))
                }
                */
                
            }
            catch(err) {
                console.log('PWRK_SHARE::'+JSON.stringify(err))
            }
