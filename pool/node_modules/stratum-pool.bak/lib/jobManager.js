...
const ExtraNonceCounter = function (configInstanceId) {
    ...
    const instanceId = configInstanceId || crypto.randomBytes(2).readUInt16LE(0) // changed from readUInt32LE;
    let counter = instanceId; // << 27; wtf bit shifting for?

    this.next = function () {
        const extraNonce = util.packUInt32BE(Math.abs(counter++));
        return extraNonce.toString('hex').substr(4); // ignore left 0000
    };
...
const JobManager = module.exports = function JobManager(options) {
    ...
    this.iid = options.instanceId
    this.extraNonceCounter = new ExtraNonceCounter(options.instanceId);
    this.extraNoncePlaceholder = new Buffer('f000000ff111111f', 'hex');
    this.extraNonce2Size = this.extraNoncePlaceholder.length - this.extraNonceCounter.size;
    console.log('xn2sz::'+this.extraNoncePlaceholder.length+'/'+this.extraNonceCounter.size)
    ...
    this.processTemplate = function (rpcData) {
        ...
        // console.log('blocktemplate::'+JSON.stringify(tmpBlockTemplate)); // debug
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName) {
        ...
        if (extraNonce2.length / 2 !== _this.extraNonce2Size) {
            // console.log('ERR_xn2::'+extraNonce2)
        ...
        if (nonce.length !== 16) {
            // console.log('ERR_xn1sz::'+nonce.length)
        ...
        if (job.target.ge(fh)) {
            ...
            // if (fh / BigInt(4000000000) > job.target) { 
                console.log('ERR_lowdiff::'+fh+','+BigInt(difficulty)+','+job.target)
